...
// deptid date period username description userid eventname roleid
if(isset($_GET['deptid']) && isset($_GET['date']) && isset($_GET['period']) && isset($_GET['username']) && isset($_GET['description']) && isset($_GET['userid']) && isset($_GET['eventname']) && isset($_GET['roleid'])) {
    ...
    if($roleid == 2) {
        ...
        if ($row2 = mysqli_fetch_array($result2)) {
            ...
            for($x=0; $x < $periodcount; $x++) {
                ...
                if($row1 = mysqli_fetch_array($result1)) {
                    ...
                    $sql2 = "select user_id, email from tbl_staff where user_name = '".$username."'";
                    $result2 = mysqli_query($DB,$sql2);
        ...
        $sql1 = "select hod from tbl_dept where dept_id = ".$deptid;
        $result1 = mysqli_query($DB,$sql1);
        $row1 = mysqli_fetch_array($result1);
        $hod = $row1['hod'];

        
        $sql4 = "select email from tbl_staff where name = '".$hod."'";
        $result4 = mysqli_query($DB,$sql4);
        $row4 = mysqli_fetch_array($result4);
        $useremail = $row4['email'];


        $sql2 = "select name from tbl_staff where user_name = '".$name."'";
        $result2 = mysqli_query($DB,$sql2);
        $row2 = mysqli_fetch_array($result2);
        $fullname = $row2['name'];
                ...
                <div class="conatiner">
                    <div class="row  pt-5" >
                        <div class="col-md-8 shadow rounded offset-md-2 col-sm-10 offset-sm-1 bg-white pt-4" >
                            ...
                            <div class="row">
                                <div class="col m-2 p-4">
                                    <p><span class="font-weight-bold">Hello  <b>'.$hod.'</b></span>,<br><br>
                                    <span class="font-weight-bold"><b>'.$fullname.'</b></span> Requested to book the seminar hall for the event (<span class="text-warning"> Event Name '.$eventname.' Event Description - '.$description.' Date - '.$date.'  for the periods '.$period.'</span>)
                                    <br> 
                                    <hr>
                                </div>
                            </div>
                            ...
                            <div class="row pb-4">
                                <div class="col text-center">
                                    <a href="http://localhost/seminar/restapi/event/allowevent.php?deptid='.$deptid.'&date='.$date.'&period='.$period.'&name='.$name.'&description='.$description.'&userid='.$userid.'&eventname='.$eventname.'&roleid='.$roleid.'&hod='.$hod.'&fullname='.$fullname.'" class="btn button text-p btn-success rounded pl-5 pr-5  text-white shadow">Allow</a>&nbsp;
                                    <a href="http://localhost/seminar/restapi/event/denyevent.php?deptid='.$deptid.'&date='.$date.'&period='.$period.'&name='.$name.'&description='.$description.'&userid='.$userid.'&eventname='.$eventname.'&roleid='.$roleid.'&hod='.$hod.'&fullname='.$fullname.'" class="btn button text-d btn-danger text-white rounded shadow pl-5 pr-5">Deny</a>
                                </div>
                            </div>
                            <br>
                            <br>
